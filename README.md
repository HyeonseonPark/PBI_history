...
| Rubus_Pangenome_Project | NAS_Archive | De novo genome assembly | *Rubus*(산딸기속) 4개 근연종(Rco/Rcra/Rpho/Rta) haplotype-resolved de novo 판게놈 조립 | hifiasm v0.25.0-r726(4종 공통), oatk v1.0(4종 공통), bwa 0.7.19-r1273+samblaster 0.1.26+YaHS(Rco/Rcra/Rpho, purge level 3 + no_ec 스캐폴딩), FastK(Rco/Rcra/Rpho) / jellyfish+GenomeScope v1.0(Rta), AutoHiC(Rta, Juicer v1.6+3D-DNA 180922) | Rco/Rcra/Rpho 3종은 hifiasm(purge3)+YaHS(no_ec)로 동일 스캐폴딩 파이프라인 사용, Rta만 AutoHiC 사용. de novo 조립 이후 후속 분석은 별도 문서 3개로 분리 — comparative_genomics.md, evolutionary_genomics.md, genome_annotation_and_validation.md 참조 |
