...
`7_CAFE/`에는 계통수를 만드는 방식이 서로 다른 두 변형이 남아있다: OrthoFinder가 자체 산출한 종트리를
그대로 쓰는 `OrthoFinder_Tree/` 경로와, 11종 단일카피 오솔로그를 mafft+Gblocks로 직접 재정렬해
RAxML로 트리를 새로 추정한 `Manual_Tree/` 경로다. 두 경로 모두 CAFE5까지 끝까지 실행되어 결과 파일이
남아있으나, GO term 주석·`Zs_Expansion/Contraction` 계통별 추출 파일·`Mafft_Gb_Raxml_cafe5/` 산출물은
`Manual_Tree/` 쪽에만 존재하고(가장 마지막 수정 시각도 2024-07-03으로 `OrthoFinder_Tree/`의
2024-06-20보다 늦다), `8_MCMC_Tree/Manual_Tree/`도 동일한 RAxML 트리를 입력으로 재사용하고 있다. 이
정황들로 볼 때 최종적으로 채택되어 해석에 쓰인 것은 `Manual_Tree` 경로이며, `OrthoFinder_Tree` 경로는
비교/검증용 병행 시도로 판단된다(다만 이 판단의 근거가 되는 명시적 결정 문서는 없음 — 확인 필요).
`7_CAFE/Spartina/`는 Spartina alterniflora를 추가 외군으로 포함한 확장판 CAFE5 재실행(p-value
0.05/0.01 두 버전)으로, Zoysia_sinica_Genome_Assembly 프로젝트 내부의 후속 변형이다.
...
- **소프트웨어(버전):** CAFE5(버전 확인 필요 — 실행 커맨드 자체는 `Spartina/` 로그에서만 확인되고
  Manual_Tree/OrthoFinder_Tree 두 채택 경로에는 cafe5 로그가 남아있지 않음)
- **핵심 파라미터:** Base 모델(오솔로그그룹별 birth-death 모델), Lambda는 Manual_Tree 경로에서
  0.00625(최대 가능값과 동일, `Base_results.txt`), OrthoFinder_Tree 경로에서 1.93099로 서로 다르게
  추정됨(트리 branch length 스케일이 다르기 때문). `Spartina/` 확장판은
...
- **Input:** Manual_Tree — `filtered_cafe_input.txt`(오솔로그그룹별 종 카운트 테이블, comparative_genomics.md
  OrthoFinder 결과 기반) + Step 2의 `RAxML_bestTree.*.ultrametric.tre`. OrthoFinder_Tree —
  동일 `filtered_cafe_input.txt` + `SpeciesTree_rooted.rename.txt.ultrametric.tre`(OrthoFinder 자체
  종트리를 ultrametric 변환한 버전, root=160 동일). Spartina 확장판 — `Orthogroups.GeneCount.tsv` 기반
  재계산된 `filtered_cafe_input.txt`(12종, Spartina alterniflora 추가) + `SpeciesTree_rooted.txt`
- **Output:** `Base_report.cafe`, `Base_results.txt`, `Base_family_results.txt`(오솔로그그룹별 p-value),
  `Base_clade_results.txt`, `Base_asr.tre`; Manual_Tree 경로에는 추가로 `Zs_Expansion_ID.txt` /
  `Zs_Contraction_ID.txt` / `Zs_specific_Expansion_ID.txt`와 이를 GO term에 매핑한
  `Zs_Expansion_Significant.ZsGOTerm` 등 Zoysia_sinica 계통 특이 확장/축소 유전자 목록 산출물이 존재
- **실행 커맨드:**
  ```bash
  ...
  # Manual_Tree / OrthoFinder_Tree 채택 경로의 실제 cafe5 실행 커맨드 자체는 로그 파일 없음 — 확인 필요
  # (입력 파일 이름과 Base_report.cafe 트리 구조로 미루어 각각의 ultrametric 트리 + filtered_cafe_input.txt를
...
- 참고: `1st_11sp/`(11종 구성, 2024-07-07)와 `2nd_6sp/`(6종 구성, 2024-07-09)는 최종 채택된 7종
  구성(top-level, 2024-07-09~07-16 및 05_branchsite_model_* 2024-07-10~08-02)보다 앞서 수행된 이전
  시행착오 버전으로 판단된다(디렉토리 수정시각 순서: 1st_11sp → 2nd_6sp → top-level).
...
- 프로젝트 내부의 분기/변형:
  - CAFE5 입력 트리를 두 가지 방식(OrthoFinder 자체 종트리 vs mafft+Gblocks+RAxML로 재구축한 트리)으로
    병행 시도했고, 최종 해석(GO term 매핑, 계통 특이 확장/축소 유전자 추출)은 `Manual_Tree`(RAxML) 경로
    산출물만을 사용했다.
  - CAFE5는 이후 Spartina alterniflora를 추가 외군으로 포함한 12종 확장판(p-value 0.05/0.01 두 버전)으로
    한 차례 더 재실행되었다 — 별도 프로젝트가 아니라 동일 프로젝트 내 후속 확장.
  - PSG(양성선택) 분석은 처음에 11종 구성(`1st_11sp/`), 이어서 6종으로 축소한 구성(`2nd_6sp/`)을
    시도한 뒤, 최종적으로 Spartina alterniflora를 포함한 7종 구성(top-level `01_`~`05_` 폴더)으로
    정착했다. 전경 계통도 처음엔 Zs/ZsSa 2개만 실행했다가 이후(2024-08-02) Zj/Zmt/Zp 3개 계통을 추가해
    총 5개 계통 각각에 대해 독립적으로 branch-site 검정을 반복했다.
...
|---|---|---|---|
| Zoysia_sinica_Genome_Assembly | CAFE5용 계통수를 OrthoFinder 자체 트리와 mafft+Gblocks+RAxML 재구축 트리 두 가지로 병행 시도 후 RAxML 경로를 채택; PSG는 11종→6종→7종(Spartina 포함) 순으로 구성을 좁혀가며 최종 5개 전경 계통(Zs/ZsSa/Zj/Zmt/Zp)에 대해 반복 실행 | RAxML 8.2.12, Gblocks 0.91b, PAML(mcmctree 4.10.7/4.10.6 혼용, codeml 4.10.7) — CAFE5/mafft/translatorX/pal2nal 버전은 확인 필요 | Spartina alterniflora를 외군으로 추가한 12종 CAFE5 확장판(P0.05/P0.01); ZsSa(Zoysia_sinica+Spartina 동시 전경) branch-site 모델 |
