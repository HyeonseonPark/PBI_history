# 진화유전체 분석 (Zoysia_sinica_Genome_Assembly, Rubus_Pangenome_Project)
...
- 연구목적/샘플 설계: Rubus속 근연 4종(Rco/Rcra/Rpho/Rta, 각 종 hap1/hap2 haplotype-resolved 조립)
  pangenome 프로젝트로, 서로 독립적인 3개 비교유전체 분석 스레드를 수행한다 — ① OrthoFinder 단일카피
  오솔로그 기반 PAML mcmctree Bayesian relaxed-clock 분기시간 추정, ② panTE BLAST 기반 LTR
  retrotransposon 삽입연대(burst-history) 분석, ③ SubPhaser k-mer 기반 서브게놈(subgenome) phasing
  분석.
- 공통 파이프라인과의 차이점: Zoysia_sinica_Genome_Assembly와 소프트웨어 스택·입력 데이터·산출물을
  공유하지 않는 독립된 프로젝트다. 분기시간 추정에 PAML mcmctree의 동일한 Hessian(usedata=3)→
  MCMC(usedata=2) 절차를 사용한다는 점이 유일한 방법론적 접점이다. Rubus_Pangenome_Project에는
  CAFE5 유전자가족 진화나 PSG(양성선택) 분석에 해당하는 스레드가 없는 대신, Zoysia_sinica_Genome_Assembly에는
  없는 LTR burst-history 및 SubPhaser 서브게놈 phasing 분석이 존재한다.
...
| Rubus_Pangenome_Project | CAFE5/PSG 스레드 없이 분기시간 추정(PAML mcmctree)·LTR burst-history·SubPhaser 서브게놈 phasing 3개 독립 스레드로 구성 | PAML mcmctree 4.10.10(단일 버전, Zoysia_sinica의 4.10.7/4.10.6 혼용과 대조) | panTE BLAST 기반 LTR retrotransposon 삽입연대 분석, SubPhaser k-mer 기반 서브게놈 phasing 분석 |
