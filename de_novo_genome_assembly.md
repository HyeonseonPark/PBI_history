...
- 공통 파이프라인과의 차이점:
  ...
  - hifiasm v0.25.0-r726(4종 공통, 로그의 버전 문자열로 확인)에 Omni-C reads를 `--h1`/`--h2`로
    직접 투입해 haplotype-resolved 조립을 수행하는 방식은 4종 공통이다.
  - 오가넬(엽록체/미토콘드리아) 분리는 4종 모두 oatk v1.0(OatkDB v20230921 magnoliopsida
    mito/pltd 모델)을 사용하며, 실행 커맨드가 각 종 로그에서 동일한 패턴으로 확인된다.
  - Hi-C 스캐폴딩은 Rco/Rcra/Rpho가 yahs를 실행했고, Rta는 AutoHiC(Juicer+3D-DNA+딥러닝
    오류교정/염색체배정 모델 통합 파이프라인)를 사용했다(`juicer.log`: Juicer version 1.6,
    BWA 0.7.17-r1188; `3d-dna.log`: 버전 180922로 확인).
  - Rta의 AutoHiC 최종 산출물(`result/AutoHiC/Rta_hap1_autohic.fasta`)에 대한 QUAST 결과:
    5개 contig, Total length 254,145,080 bp, N50 130,016,229 bp, GC 37.79%(`report.txt`에서
    직접 확인).
- 실행 커맨드(로그/스크립트에서 직접 확인된 것만):
  ```bash
...
| Rubus_Pangenome_Project | 4개 근연종(Rco/Rcra/Rpho/Rta) 각 hap1/hap2 haplotype-resolved 조립. hifiasm에 Omni-C reads를 `--h1/--h2`로 투입해 haplotype 분리 조립(4종 공통), oatk로 오가넬 분리(4종 공통). Hi-C 스캐폴딩은 Rco/Rcra/Rpho가 yahs, Rta는 AutoHiC(Juicer+3D-DNA+딥러닝) 사용 | hifiasm v0.25.0-r726(로그에서 직접 확인, 4종 공통), oatk v1.0(4종 공통), FastK(Rco/Rcra/Rpho) vs jellyfish+GenomeScope v1.0(Rta), Juicer v1.6+BWA 0.7.17-r1188+3D-DNA 180922(Rta AutoHiC, 로그에서 직접 확인) | de novo 조립 이후 후속 분석은 별도 문서로 분리 — comparative_genomics.md, evolutionary_genomics.md, genome_annotation_and_validation.md 참조 |
