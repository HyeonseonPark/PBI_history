# De novo Genome Assembly (Cannabis_sativa_cv.ChungSam_dnDNAseq, Pichia_Pastoris_dnDNAseq_HanHwa, Abies_koreana_dnDNAseq, Zoysia_macrostachya_dnDNAseq, Zoysia_sinica_Genome_Assembly)
...
### Step 5. Hi-C(Omni-C) 기반 de novo 스캐폴딩 — Zoysia_macrostachya/Zoysia_sinica만 해당
두 프로젝트 모두 Omni-C(Hi-C)를 이용한 de novo 스캐폴딩이라는 접근 자체는 같지만, 실제 사용한
...
- (해당 시) 프로젝트 고유 확장 분석: Zoysia_macrostachya와 매우 유사한 폭의 비교유전체/하류 분석 스위트를 보유하나, 하나의 `Pan-genome_analysis/`로 통합되어 있지 않고 번호가 매겨진 개별 폴더(`5_`~`15_` 등)로 나뉘어 있다.
  - `5_Functional_annotation`/`EnTAP`: EnTAP 기반 기능주석(GO/EC/Pathway) 및 Arabidopsis/Oryza 유사도 비교, Zoysia 속 여러 종의 BRAKER codingseq 기능주석.
  - `6_OrthoFinder`: 여러 Zoysia속 종(및 Spartina 등 외군) 단백질 세트로 오솔로그 그룹 클러스터링.
  - `7_CAFE`: OrthoFinder 오솔로그그룹 수 변화 기반 유전자 패밀리 확장/축소(CAFE) 분석.
  - `8_MCMC_Tree`: PAML mcmctree 기반 분자시계 분기시간 추정(Bayesian).
  - `9_Ks_distribution`: MCScanX 공선성 블록 기반 Ka/Ks 분석 및 전유전체중복(WGD) Ks peak 시각화.
  - `10.Positive-selected-gene`: translatorX→mafft→pal2nal→Gblocks→PAML branch-site model+LRT 기반 계통별 양성선택 유전자 탐색.
  - `11_Waterlogging_genes`: 침수(waterlogging) 관련 후보 유전자(AP2 도메인 등) 아미노산 변이 비교.
  - `12_Circos`: 최종 게놈의 karyotype/LTR 분포 및 공선성 블록 Circos 시각화.
  - `13_TeloExplorer`(quarTeT): contig-level 및 Manualcurated 게놈에 대한 텔로미어 반복서열 탐지.
  - `14_CentroMiner`: Tandem Repeat 기반 센트로미어 후보 영역 탐지.
  - `15_PlantCARE`: 오솔로그그룹별 TSS 상류 서열 추출 후 PlantCARE cis-regulatory element 예측.
  - `qRT-PCR`: 후보 유전자 대상 qRT-PCR 검증용 프라이머 후보 필터링/시각화.
  - `Zoysia_Gene_reannotation`: Z. japonica/matrella/pacifica 등 근연종 및 Zoysia_sinica contig-level 게놈에 대한 재주석(IsoSeq 병용 시도 포함) — 이 문서의 핵심 파이프라인(Step 9)이 사용한 최종 BRAKER3 실행과는 별개의 실험/비교용 실행으로 추정된다.
